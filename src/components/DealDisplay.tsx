...
const DealDisplay = () => {
  ...
  if (allProducts.loading) {
    ...
    const products: Product[] = allProducts.data.allProducts.filter((d: { isDeal: boolean }) => d.isDeal === true)
    ...
    const dealDisplay = products.map(p =>
          ...
          <Paper elevation={5} style={{ backgroundColor: blk }}>
            <div className={classes.section1}>
              ...
              <Typography style={{ color: txt }} variant="body2">
                {p.type === 'drink' ? `Order cases of your favorite soft drinks direct from the manufacturer!` : `High quality t-shirts with your favorite brand logo front and center.`}
              </Typography>
            </div>
            ...
            <div className={classes.section3} style={{ width: '100%' }}>
              <FormControl variant="outlined">
                <InputLabel htmlFor="outlined-quantity-native-simple" style={{ fontSize: 14, lineHeight: 0.25, color: bg }}>{p.type === 'drink' ? `Cases` : `Quantity`}</InputLabel>
                <Select
                  id={p.id}
                  native
                  value={(p.id === idQuantity) ? quantity : ''}
                  onChange={selectQuantity}
                  style={p.type === 'drink' ? { width: 80, height: 40, marginRight: 20, backgroundColor: txt } : { width: 100, height: 40, marginRight: 20, backgroundColor: txt }}
                  label="Cases"
                  inputProps={{
                    name: `${p.id}`,
              ...
              <FormControl variant="outlined">
                <InputLabel htmlFor="outlined-quantity-native-simple" style={{ fontSize: 14, lineHeight: 0.25, color: bg }}>{p.type === 'drink' ? `Flavor` : `Color`}</InputLabel>
                <Select
                  id={p.id}
                  native
                  value={p.type === 'drink' && p.id === idFlavor ? flavor : p.type === 'TShirt' && p.id === idColor ? color : ''}
                  onChange={p.type === 'drink' ? selectFlavor : selectColor}
